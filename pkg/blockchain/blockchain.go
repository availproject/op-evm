...
func NewBlockchain(
	...
	b.stream.push(&blockchain.Event{})
...
func (b *Blockchain) writeCanonicalHeader(event *blockchain.Event, h *types.Header) error {
	...
	event.Type = blockchain.EventHead
...
func (b *Blockchain) WriteFullBlock(fblock *types.FullBlock, source string) error {
	...
	evnt := &blockchain.Event{Source: source}
...
func (b *Blockchain) dispatchEvent(evnt *blockchain.Event) {
	b.stream.push(evnt)
}

// writeHeaderImpl writes a block and the data, assumes the genesis is already set
func (b *Blockchain) writeHeaderImpl(evnt *blockchain.Event, header *types.Header) error {
	...
	if incomingTD.Cmp(currentTD) > 0 {
		...
		evnt.Type = blockchain.EventFork
...
func (b *Blockchain) handleReorg(
	...
	progress := true
	for oldHeader.Hash != newHeader.Hash && progress {
		// Start with progress as false and only flip it if either of the headers get
		// updated, to avoid infinite loop.
		progress = false

		if oldHeader.Number > newHeader.Number {
			oldHeader, ok = b.readHeader(oldHeader.ParentHash)
			if !ok {
				return fmt.Errorf("header '%s' not found", oldHeader.ParentHash.String())
			}
		...
		if len(oldChain) == 0 || oldChain[len(oldChain)-1].Hash != oldHeader.Hash {
			oldChain = append(oldChain, oldHeader)
		}
	}
	...
	evnt.Type = blockchain.EventReorg
